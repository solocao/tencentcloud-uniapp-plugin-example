    ...
    <view class="content">
      <view class="form-item">
        ...
        <radio-group @change="serverTypeChange">
          <label class="radio">
            <radio value="0" :checked="true" :disabled="status" />英文</label>
          <label class="radio">
            <radio value="1" :disabled="status" />中文</label>
        </radio-group>
      </view>
      <view class="form-item">
        ...
        <radio-group @change="scoreCoeffChange">
          <label class="radio">
            <radio value="1" checked="true" :disabled="status" />儿童</label>
          <label class="radio">
            <radio value="3" :disabled="status" />成人</label>
        </radio-group>
      </view>
      <view class="form-item">
        ...
        <radio-group @change="evalModeChange">
          <label class="radio">
            <radio value="0" checked="true" :disabled="status" />词模式</label>
          <label class="radio">
            <radio value="1" :disabled="status" />句子模式</label>
          <label class="radio">
            <radio value="2" :disabled="status" />段落模式</label>
          <label class="radio">
            <radio value="3" :disabled="status" />自由说模式</label>
        </radio-group>
      </view>
      <view class="form-item">
        ...
        <radio-group @change="workModeChange">
          <label class="radio">
            <radio value="1" checked="true" :disabled="status" />非流式</label>
          <label class="radio">
            <radio value="0" :disabled="status" />流式分片</label>
        </radio-group>
      </view>
      <view class="form-item">
    ...
    methods: {
      ...
      refTextChange() {
        if(this.option.serverType == serverType.en) {
          ...
          if(this.option.evalMode == evalMode.sentence || this.option.evalMode == evalMode.freeSpeak) this.refText = "I like dog";
        } else {
          if(this.option.evalMode == evalMode.word) this.refText = "狗";
          if(this.option.evalMode == evalMode.sentence || this.option.evalMode == evalMode.freeSpeak) this.refText = "我喜欢狗";
        }
      },
      workModeChange(ev) {
      ...
      async startRecord() {
        try {
          ...
          if(this.option.workMode == workMode.stream) { // 流式
