...
export default {
  ...
  handleAssessQuality: async function () {
    try {
      ...
      this.orcResults = [
        `美观度：${result.AestheticScore}`,
        `清晰度：${result.ClarityScore}`,
      ];
  ...
  // 恶心检测
  handleDetectDisgust: async function () {
    try {
      const img = await chooseImage2Base64();
      uni.showLoading({
        mask: true,
  ...
  // 不良行为识别
  handleDetectMisbehavior: async function () {
    try {
      const img = await chooseImage2Base64();
      uni.showLoading({
        mask: true,
  ...
  // 图像标签
  handleDetectLabel: async function () {
    try {
      const img = await chooseImage2Base64();
      uni.showLoading({
        mask: true,
  ...
  // 车辆识别
  handleRecognizeCar: async function () {
    try {
      const img = await chooseImage2Base64();
      uni.showLoading({
        mask: true,
  ...
  // 商品识别
  handleDetectProduct: async function () {
    try {
      const img = await chooseImage2Base64();
      uni.showLoading({
        mask: true,
  ...
  // 公众人物识别
  handleDetectCelebrity: async function () {
    try {
      const img = await chooseImage2Base64();
      uni.showLoading({
        mask: true,
  ...
  // 图像清晰度增强
  handleEnhanceImage: async function () {
    try {
      const img = await chooseImage2Base64();
      uni.showLoading({
        mask: true,
  ...
  // 图片智能裁剪
  handleCropImage: async function () {
    try {
      const img = await chooseImage2Base64();
      uni.showLoading({
        mask: true,
